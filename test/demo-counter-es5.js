			...
			value: function update() {
				...
				try {
					for (var _iterator11 = this.stops[Symbol.iterator](), _step11; !(_iteratorNormalCompletion11 = (_step11 = _iterator11.next()).done); _iteratorNormalCompletion11 = true) {
						var _stop = _step11.value;

						updateStop(scroll, _stop);
					}
				} catch (err) {
			...
			value: function nearestStop(position) {
				...
				try {
					for (var _iterator15 = this.stops[Symbol.iterator](), _step15; !(_iteratorNormalCompletion15 = (_step15 = _iterator15.next()).done); _iteratorNormalCompletion15 = true) {
						var _stop4 = _step15.value;


						if (type !== null && type !== _stop4.type) continue;

						var d = mode === 'near' ? Math.abs(position - _stop4.position) : mode === 'prev' ? position > _stop4.position ? position - _stop4.position : NaN : mode === 'next' ? position < _stop4.position ? _stop4.position - position : NaN : NaN;
 ...
 */
	...
	try {
		for (var _iterator21 = document.querySelectorAll('.counter .digit').entries()[Symbol.iterator](), _step21; !(_iteratorNormalCompletion21 = (_step21 = _iterator21.next()).done); _iteratorNormalCompletion21 = true) {
			var _step21$value = _slicedToArray(_step21.value, 2),
			    index = _step21$value[0],
			    element = _step21$value[1];
